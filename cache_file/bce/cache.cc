...
{
  // cout<<"clt size: "<<c_loc_table.size()<<"  ";
...
uint32_t CACHE::cluster_indirection_module(uint32_t cpu, uint32_t LCID, vector<uint32_t> line_address)
{
    ...
    // cout<<" set_id: "<<set_id<<" lcid: "<<LCID<<" lcid_0: "<<LCID_0<<" PCID: "<<PCID<<" CLUSTER_OFFSET: "<<cluster_offset_<<endl;
...
uint32_t CACHE::get_set(uint64_t address, uint32_t cpu)
{
    if(cache_type == IS_LLC)
    {
        ...
        // cout<<"addr: "<<address<<"    32 bit_address: "<<addr<<"   line_addr: ";
        // for ( int i = 0; i < line_address.size(); i++ ) 
        // {
        //     cout<<line_address[i];
        // }
