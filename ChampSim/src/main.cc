...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout << cache->NAME;
    cout << " WRITEBACK FROM ABOVE LEVEL ACCESS: " << setw(10) << cache->roi_access[cpu][3] << "  HIT: " << setw(10) << cache->roi_hit[cpu][3] << "  MISS: " << setw(10) << cache->roi_miss[cpu][3] << endl;

    cout << cache->NAME;
    cout << " WRITEBACK TO BELOW LEVEL ACCESS: " << setw(10) << cache->roi_access[cpu][4] << "  HIT: " << setw(10) << cache->roi_hit[cpu][4] << "  MISS: " << setw(10) << cache->roi_miss[cpu][4] << endl;
...
int main(int argc, char** argv)
{
    ...
    while (run_simulation) {
        ...
        // uncore.LLC.bandwidth_calc(0,&uncore.LLC);
